...
import org.dsacleveland.evictiontracker.model.evictiondata.dto.AttorneyDto;
import org.dsacleveland.evictiontracker.model.evictiondata.dto.CaseDto;
import org.dsacleveland.evictiontracker.model.evictiondata.dto.EventDto;
...
public class PdfCaseParserImpl implements PdfCaseParser {
    ...
    public List<CaseDto> parseCasesFromPdfText(String pdfText) {
        final List<String> rawLines = Arrays.asList(pdfText.split("\n"));
        ...
        // Gets all lines excluding "Legal news" header lines
        ...
        // Index (in 'lines') of the CVG line (start of a Case record)
        List<Integer> partitionIndices = Stream
                .concat(lines
                        .stream()
                        .filter(line -> line.matches("20(\\d{2}) CVG (\\d+).+"))
    ...
    private Optional<CaseDto> processCasePartition(List<String> partition) {
        ...
        try {
            ...
            System.err.println("Encountered malformed data, on " + partition + ", skipping");
            return Optional.empty();
        }

        List<List<String>> segments;
        try {
            segments = this.partition(partition, partitionIndices);
        } catch (Exception e) {
            System.err.println(partition.toString());
            System.err.println(partitionIndices.toString());
            throw e;
        }
        ...
        try {
            ...
            if (caseNumberMatcher.matches()) {
                ...
                System.err.println("Could not parse case number from " + segments.get(0));
            }
            ...
            if (dateMatcher.matches()) {
                ...
                System.err.println("Could not parse case file date from " + segments.get(4));
            }
            caseDto.setEvents(this.processEventsSubpartition(segments.get(5)));
        } catch (RuntimeException e) {
            System.err.println("Issue loading case file with info " + partition);
    ...
    private Optional<PartyDto> processPartySubpartition(List<String> subpartition) {
        if (subpartition.size() < 3) {
            System.err.println("Could not read badly formed party " + subpartition);
            return Optional.empty();
        }
        ...
        if (!csz.matches()) {
            System.err.println("Could not parse city, state, and/or zip code for party " + subpartition);
            return Optional.empty();
        } else {
    ...
    // Returns a list of lists, which are the items of the input list
    // partitioned by partition indices
